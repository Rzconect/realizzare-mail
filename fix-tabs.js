...
const timelineBlockStart = `<section className="lg:col-span-4 lg:h-full lg:overflow-y-auto scrollbar-none p-1">
...
const replacement = `<section className="lg:col-span-4 lg:h-full lg:overflow-y-auto scrollbar-none p-1">
...
c = c.replace(timelineBlockStart, replacement);

const timelineBlockEnd = `                  )}
                </div>
              </div>
            </div>
          </section>`;
...
c = c.replace(timelineBlockEnd, replacementEnd);

fs.writeFileSync('src/app/dashboard/contacts/[id]/page.tsx', c);
console.log('Added Right Panel Tabs');
