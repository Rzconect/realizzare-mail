...
export async function GET() {
  try {
    ...
    // Match pending and paid events robustly
    ...
    for (const p of truePending) {
       ...
       if (!hasDuplicate) uniquePending.push(p);
          ...
          // Keep best title & merge states
          const dupe = uniquePending[uniquePending.indexOf(hasDuplicate)];
          const existingTitle = dupe.metadata?.item_title || "";
          ...
          if (newTitle && !newTitle.includes("Certificado / Curso") && existingTitle.includes("Certificado / Curso")) {
             dupe.metadata = { ...dupe.metadata, item_title: newTitle };
          }
          if (p.metadata?.crm_column) dupe.metadata.crm_column = p.metadata.crm_column;
          if (p.metadata?.crm_assigned) dupe.metadata.crm_assigned = p.metadata.crm_assigned;
       }
    }

    const activePending = uniquePending.slice(0, 50);
